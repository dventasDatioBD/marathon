...
Networking and port configuration for applications in Marathon can be confusing and this page attempts to explain more clearly how they work.
Marathon continues to support the [legacy ports API](ports-legacy.html) that was shipped in versions 1.4.x and prior.
All new applications should be declared using the new, non-deprecated networking API fields.
...
Support for deprecated networking API fields will be dropped completely in some future release according to Marathon's official API deprecation policy.
...
* `MIGRATION_1_5_0_MARATHON_DEFAULT_NETWORK_NAME` is used when upgrading Marathon from a version prior to v1.5.
  * older MESOS IP/CT app definitions were not required to declare an `ipAddress/networkName`; this has changed with Marathon v1.5
  * migration automatically configures `container` networking mode for each migrated legacy MESOS IP/CT app
  * at migration time, legacy MESOS IP/CT app definitions are configured to use the network name defined by the above migration-specific environment variable
  * if the environment variable is **unset** then migration uses the network name defined by the `--default_network_name` flag
  * if both the environment variable and flag are **unset** then app migration will fail.
...
Note that only host ports are made available to a task through environment variables.
...
* If unspecified (`null`) at create-time, defaults to `{ "portDefinitions": [ { "port": 0, "name": "default" } ], ... }`
...
If you have set `containerPort` to `0` then this will be the same as `hostPort` and you can use the `$PORTxxx` environment variables.
...
* `labels` may defined for items of `portDefinitions` as well as for items of `portMappings`, and are sent to Mesos via `DiscoveryInfo` protobufs at instance-launch time.
...
Note that for apps making use of Docker containers, it not necessary to `EXPOSE` ports in your `Dockerfile`.
...
It is important to note that the `$PORT_xxx` variables refer to the host ports.
